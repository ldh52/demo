...
public class BookServiceImplTest {
    ...
    @InjectMocks
    private BookServiceImpl bookService;   // BookServiceImpl 객체 생성하고, bookRepository 주입

    @BeforeEach
        // 매 테스트 메소드 실행할 때 마다 수행
    void setup() {
    ...
    void testGetBookList() {
        Book book1 = Book.builder().bid(999L).title("title1").author("author1").company("company1")
            .summary("summary1").build();
        Book book2 = Book.builder().bid(1000L).title("title2").author("author2").company("company2")
            .summary("summary2").build();
        List<Book> bookList = Arrays.asList(book1, book2);
        Pageable pageable = PageRequest.of(0, BookService.PAGE_SIZE);
        Page<Book> bookPage = new PageImpl<>(bookList, pageable, bookList.size());
