...
public class CsvFileReaderService {
    ...
    public void csvFileToH2() {
        try {
            Resource resource = resourceLoader.getResource(
                "classpath:/static/data/20241114_yes24_국내도서_새로나온_상품.csv");
    ...
    public void csvFileToElasticSearch() {
        try {
            Resource resource = resourceLoader.getResource(
                "classpath:/static/data/yes24_국내도서_새로나온_상품.csv");
            try (Reader reader = new InputStreamReader(resource.getInputStream(),
                StandardCharsets.UTF_8);
                CSVParser csvParser = new CSVParser(reader,
                    CSVFormat.DEFAULT.withFirstRecordAsHeader())) {
            ...
            try (Reader reader = new InputStreamReader(resource.getInputStream(),
                ...
                for (CSVRecord record : csvParser) {
                    ...
                    try {
                        ...
                        List<Map<String, Object>> reviews = objectMapper.readValue(reviewJsonStr,
                            new TypeReference<>() {
                            });
